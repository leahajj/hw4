...
static void handle_client(int connfd){
    // Handshake: expect "DEC\n"
    char hello[4];
    if (recv_all(connfd, hello, 4) <= 0 || memcmp(hello, "DEC\n", 4) != 0) {
    ...
    // Receive lengths
    uint32_t clen=0, klen=0;
    if (!recv_u32(connfd, &clen) || !recv_u32(connfd, &klen) || klen < clen) {
        close(connfd);
        _exit(0);
    }

    // Receive payloads (ciphertext and key)
    char* ctext = malloc(clen);
    char* ktext = malloc(klen);
    if (!ctext || !ktext) { free(ctext); free(ktext); close(connfd); _exit(0); }

    if (recv_all(connfd, ctext, clen) != (ssize_t)clen ||
        recv_all(connfd, ktext, klen) != (ssize_t)klen) {
        free(ctext); free(ktext); close(connfd); _exit(0);
    }

    // Decrypt: p[i] = (c[i] - k[i] + 27) % 27
    char* ptext = malloc(clen);
    if (!ptext) { free(ctext); free(ktext); close(connfd); _exit(0); }

    for (uint32_t i = 0; i < clen; i++) {
        int cv = map27(ctext[i]);
    ...
    // Send plaintext back
    (void)send_u32(connfd, clen);
    (void)send_all(connfd, ptext, clen);
...
int main(int argc, char* argv[]){
    ...
    if (port <= 0) { fprintf(stderr, "dec_server error: bad port\n"); exit(1); }
    ...
    for (;;) {
        ...
        if (pid == 0) { close(sockfd); handle_client(connfd); }
        close(connfd);
    }
}
